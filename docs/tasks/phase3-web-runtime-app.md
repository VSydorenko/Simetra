...
**Межа платформа/проєкт:** metadata і форми живуть у каталозі проєкту користувача (зовнішній щодо репо Simetra), як це вже працює для `*.meta.json`. Simetra-платформа лише зчитує ці файли. Файлова структура за BRD §7.2:
...
flowchart TD
  ...
  LOADER --> MODEL[ProjectModel + Forms map]
...
- **Core (form model)** — Zod-схеми `form.json`, алгоритм автоформи, `resolveForm()`, shared metadata IO layer. Чистий TS + Zod, без React.
...
4. **Shared Metadata Loader** парсить JSON і валідує через Zod-схеми з `@simetra/core`. Результат: `ProjectModel` + optional `Map<string, FormSchema>`.
...
### Етап 2: Storage write-path adaptation

> **Мета:** конфігуратор не знищує `forms/` каталоги при save. Це **критичний** prerequisite для Phase 3 runtime, бо поточний `clearDirectory(metadataDir)` зітре всі explicit forms.

- [ ] Змінити `apps/web/src/storage/web-storage.ts` write-path:
  - Замість повного `clearDirectory(metadataDir)` — видаляти тільки canonical metadata files (`*.meta.json`, порожні object directories)
  - **Не чіпати** каталоги `forms/` та інші non-canonical файли всередині object directories
  - Не чіпати `.simetra/` (CLI snapshot directory)
- [ ] Альтернатива: зробити selective cleanup per kind directory — видаляти тільки `*.meta.json` файли, зберігаючи структуру каталогів з forms
- [ ] **Тести:**
  - Save не видаляє існуючий `forms/item.form.json` у каталозі об'єкта
  - Save видаляє stale `*.meta.json` для видалених об'єктів
  - Save не чіпає `.simetra/` каталог
  - Rename об'єкта: старий каталог з forms видаляється (разом із forms), новий створюється
...
- [ ] Створити `apps/runtime/`:
  - `package.json` — `runtime`, залежить від `@simetra/form-runtime`, `@simetra/data-provider-postgrest`, `@simetra/core`, `@simetra/metadata-io`
...
- [ ] `apps/runtime/src/load-metadata.ts`:
  ...
  - Використовує `@simetra/metadata-io` (shared parser з Етапу 1) → `ProjectModel`
  - **Не** створювати третій парсер — імпорт з shared IO layer
...
- [ ] `src/pages/list-page.tsx`:
  - Використовує `resolveForm(objectRef, "ListForm", ...)` → `ListRenderer`
  - Кнопка "Створити" → navigate to `/new`
  - Клік по рядку → navigate to `/:id`
- [ ] `src/pages/item-page.tsx`:
  - Використовує `resolveForm(objectRef, "ItemForm", ...)` → `ItemFormRenderer`
...
- [ ] End-to-end сценарій (з PostgREST):
  ...
  2. Зберегти metadata на диск (перевірити, що `forms/` не видаляється при збереженні)
...
### Shared IO pattern
Metadata parsing logic (`ProjectModel` assembly з `*.meta.json` файлів) має жити в одному shared пакеті (`@simetra/metadata-io`). CLI, web-storage і runtime — всі делегують до нього. Жодних третіх парсерів.
...
Не створювати третій парсер metadata в `apps/runtime`. CLI вже має `read-metadata.ts` + `build-model.ts`, web має `web-storage.ts`. Новий loader має делегувати до shared `@simetra/metadata-io`.

### ❌ Storage data loss
`clearDirectory(metadataDir)` в `web-storage.ts` знищує **все** в metadata директорії перед записом. Після Phase 3 в metadata/ з'являться `forms/` директорії. Write-path **мусить** зберігати їх при збереженні.
...
- [ ] `@simetra/metadata-io` — shared parser, використовується в CLI, web-storage і runtime
- [ ] Storage write-path: `serializeToFiles()` / `clearDirectory()` зберігає `forms/` при збереженні
...
- [ ] End-to-end сценарій (mock): configurator → metadata → runtime dev preview → форми рендеряться
