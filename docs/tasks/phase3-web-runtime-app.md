...
### Етап 0: Синхронізація документації

> **Мета:** перед початком імплементації привести документацію у відповідність до архітектурних рішень Phase 3, зокрема: forms як canonical частина ProjectModel.

- [ ] Оновити `docs/BRD-metadata-configurator.md`:
  - §7.2 — file layout: підтвердити `forms/` підкаталог як canonical частину file structure
  - §10.5.2 — уточнити що `formSchema` має `objectRef: MetadataRef` для explicit прив'язки до об'єкта
  - §10.5.8 — уточнити що візуальний конструктор (Phase 4) працює з forms як частиною ProjectModel в конфігураторі, а не як окремою файловою системою
  - §10.5.9 — фазування: Phase 3 включає Zod-схеми forms і їх інтеграцію в ProjectModel
- [ ] Оновити `docs/architecture/metadata-model.md`:
  - Додати секцію «Forms model» — forms як top-level колекція ProjectModel з objectRef зв'язком
  - Описати precedence rule: explicit form > autogenerated
  - Описати які kinds підтримують forms (Catalog, Document, CustomTable)
- [ ] Оновити `docs/architecture/storage-and-persistence.md`:
  - Stale file cleanup: forms серіалізуються canonical serializer-ом як окремі файли в `forms/` підкаталозі, а не як частина `*.meta.json`
  - Save flow: `serializeToFiles()` тепер включає forms файли, повний rewrite зберігається як інваріант
  - Видалити формулювання про «non-canonical файли» стосовно forms
- [ ] Оновити `docs/architecture/ui-components.md`:
  - ObjectEditor: додати опис секції «Форми» для Catalog, Document, CustomTable
  - Tree: форми як дочірня група «Forms» під об'єктом у дереві метаданих
- [ ] Оновити `.github/instructions/metadata-model.instructions.md`:
  - Додати згадку про `formSchema` і колекцію `forms` в ProjectModel
- [ ] Оновити `.github/instructions/ui-architecture.instructions.md`:
  - Секція Forms у SECTION_CONFIG для Catalog, Document, CustomTable
- [ ] Оновити `docs/architecture/OVERVIEW.md`:
  - Відобразити shared metadata IO layer в core
  - Додати forms flow до загальної архітектурної діаграми
  - Додати нові пакети Phase 3 (form-runtime, data-provider, apps/runtime) до roadmap секції
