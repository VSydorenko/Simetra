...
**Ціль Phase 3:** Бібліотека рендерингу форм (`@simetra/form-runtime`) + unified runtime (`@simetra/app-runtime`) з progressive configuration:
...
- `@simetra/app-runtime` — єдиний runtime-пакет, який у Phase 3 працює з flat навігацією по kinds (fallback mode), а в Phase 4 розширюється subsystem routing, configured shell і dashboard;
- CRUD через DataProvider → PostgREST/Supabase або InMemoryDataProvider (dev preview без БД).
...
- Subsystem-based навігація і routing: `/{subsystem}/{object}` (Phase 4)
- Configured shell layouts (`TopNavWithTabs`, `MinimalSidebar`) (Phase 4)
...
├── app-runtime/                ← 🆕 Unified runtime (shell, routing, pages) — progressive configuration
├── data-provider/              ← 🆕 абстрактний data-access contract + InMemoryDataProvider
...
└── runtime/                    ← 🆕 thin Vite host: metadata loading + provider wiring + <SimetraApp />
```

> **Важливо:** `@workspace/ui` залишається shadcn/ui kit з примітивами. Domain-компоненти (CatalogCombobox, EnumSelect, RuntimeDataTable) живуть усередині `@simetra/form-runtime`. `@simetra/app-runtime` — shell/routing/pages бібліотека, що обгортає form-runtime. Виносити domain-компоненти в окремий `@simetra/ui` доменний пакет — Phase 4, якщо буде потреба.
...
- **App Runtime** — unified shell/routing/pages бібліотека (`@simetra/app-runtime`). У Phase 3: flat навігація з `ProjectModel`, default `SidebarWithHeader`. У Phase 4: додається subsystem routing, configured layouts, dashboard.
- **Data Provider** — абстрактний контракт для data access. Adapters: `PostgRestDataProvider` (реальна БД) і `InMemoryDataProvider` (dev preview без БД).
- **Thin Host** — `apps/runtime`, мінімальний Vite app, який з'єднує metadata loading + data provider + `<SimetraApp />`. Жодної business logic.

### `@simetra/app-runtime` — progressive configuration

| Характеристика | Phase 3 (fallback mode) | Phase 4 (configured mode) |
|----------------|------------------------|---------------------------|
| Навігація | Flat sidebar by kind з `ProjectModel` | Subsystem-based з `application.meta.json` |
| Маршрутизація | `/{kind}/{objectName}/:id` | `/{subsystem}/{object}/:id` |
| Shell layout | Default `SidebarWithHeader` | Конфігурований: `SidebarWithHeader`, `TopNavWithTabs`, `MinimalSidebar` |
| Dashboard | Немає | Widgets framework |
| Entry point | `<SimetraApp model={m} dataProvider={dp} />` | `<SimetraApp model={m} dataProvider={dp} applicationConfig={cfg} />` |
| Розповсюдження | npm-пакет `@simetra/app-runtime` | Той самий пакет, розширений |

### Як apps/runtime (thin host) працює технічно
...
6. **`<SimetraApp />`** з `@simetra/app-runtime` будує навігацію, маршрути і shell з `ProjectModel`.
7. **form-runtime** рендерить форму через React + shadcn/ui компоненти.
8. **DataProvider** спілкується з PostgreSQL через PostgREST для CRUD, або з InMemoryDataProvider для dev preview.
...
### Етап 8: `@simetra/app-runtime` — unified runtime з progressive configuration

> **Мета:** єдиний runtime-пакет, який працює як production runtime **і** як dev preview — залежно від наявності `application.meta.json`. У Phase 3 реалізується fallback mode (без application config → flat навігація з `ProjectModel`). У Phase 4 додається configured mode (subsystems, shell layouts, dashboard).
>
> **Архітектурне рішення:** замість двох окремих механізмів (`apps/runtime` як dev preview + `@simetra/app-runtime` як production shell), створюємо **один** `@simetra/app-runtime` з convention-over-configuration підходом. Це усуває throwaway code і дублювання shell/routing/pages логіки.
...
- [ ] `src/simetra-app.tsx` — головний entry point:
  ...
  />
  ```
  - Обгортає `DataProviderProvider` + `MetadataProvider` з `@simetra/form-runtime`
  - Будує навігацію і маршрути з `ProjectModel`
  - Phase 3: завжди flat nav + default shell (без `application.meta.json`)
  - Phase 4: якщо передано `applicationConfig` → subsystem nav, configured shell, dashboard
...
- [ ] `src/navigation-builder.ts`:
  - `buildFlatNavigation(model: ProjectModel): NavigationItem[]` — групує об'єкти по kind:
    ...
    - Enumeration, InformationRegister, AccumulationRegister → Phase 4 (не показувати в sidebar)
  - `NavigationItem = { kind, objectName, displayName, path }`
  - Phase 4 додасть: `buildSubsystemNavigation(model, appConfig)` — навігація по subsystems
...
- [ ] `src/router-builder.ts`:
  - `buildRoutes(model: ProjectModel): RouteObject[]` — react-router route objects:
    ...
    - `*` → `NotFoundPage`
  - Phase 4 додасть: subsystem-based routing `/{subsystem}/{object}/:id`
...
- [ ] `src/shell/sidebar-layout.tsx`:
  ...
  - Content area: `<Outlet />` для react-router
  - shadcn/ui Sidebar + Header components
  - Phase 4 додасть: `TopNavWithTabs`, `MinimalSidebar` layouts
...
- [ ] Створити `packages/data-provider/src/in-memory-provider.ts`:
  ...
  - `delete()` → видаляє з Map
  - `searchRef()` → пошук по in-memory даних (code/description fields)
  - `getRefDisplay()` → lookup display value
  - `postDocument()` / `unpostDocument()` → toggle `posted` field
  - `getConstants()` / `updateConstant()` → in-memory constants Map
  - Export з `@simetra/data-provider` barrel
  - **Reusable:** для unit-тестів form-runtime, Storybook stories, dev preview і demo

#### 8.7. `apps/runtime` — thin Vite host
...
- [ ] End-to-end сценарій (з PostgREST):
  ...
  5. `pnpm --filter runtime dev` з `SIMETRA_DATA_PROVIDER=postgrest` → побачити sidebar з об'єктами by kind
...
- [ ] End-to-end сценарій (з mock provider):
  1. `pnpm --filter runtime dev` з `SIMETRA_DATA_PROVIDER=mock`
...
- [ ] Integration-тести: `<SimetraApp />`, routing, InMemoryDataProvider
...
`apps/runtime` — це **thin host**, який лише з'єднує metadata loading + data provider + `<SimetraApp />`. Shell, routing і pages логіка живе в `@simetra/app-runtime`. Не дублювати navigation/routing/pages в `apps/runtime`.

### ❌ Third metadata loader
Не створювати окремий парсер metadata в `apps/runtime` або `@simetra/app-runtime`. Metadata loading — відповідальність хоста. `app-runtime` приймає готовий `ProjectModel` як prop.
...
`form-runtime` має працювати незалежно від `app-runtime`. Можна використати `<ItemFormRenderer>` без shell, sidebar і маршрутів. `form-runtime` — library рендерингу форм, `app-runtime` — library shell/routing/pages.
...
| Subsystem routing в `app-runtime` | `/{subsystem}/{object}` nesting потребує application config | §10.6.3 |
| Configured shell layouts | `TopNavWithTabs`, `MinimalSidebar` — Phase 4 | §10.6.4 |
| Dashboard widgets | `RecentDocuments`, `RegisterBalance`, `Counter`, `QuickLinks` — Phase 4 | §10.6.3 |
...
- [ ] `@simetra/app-runtime` — unified runtime library: `<SimetraApp />`, flat nav, default shell, standard pages
- [ ] `@simetra/data-provider` включає `InMemoryDataProvider` як reusable mock adapter
- [ ] `apps/runtime` — thin Vite host: metadata loading + provider wiring + `<SimetraApp />`
- [ ] Mock preview: `apps/runtime` з `InMemoryDataProvider` запускається без PostgreSQL
...
- [ ] End-to-end сценарій (mock): configurator → metadata (з forms) → `<SimetraApp />` з InMemoryDataProvider → форми рендеряться
- [ ] End-to-end сценарій (PostgREST): metadata → DDL → apply → `<SimetraApp />` з PostgREST → CRUD працює
