...
### Definition of Done
- [Х] Переключення між вкладками зберігає внутрішню секцію кожної вкладки
- [Х] Зірочка `*` з'являється тільки на вкладках зі зміненими об'єктами
- [Х] Після save — всі зірочки зникають
- [Х] `objectVersions` коректно обробляє rename та delete
...
### Вимоги
- [Х] Замінити горизонтальні shadcn `Tabs` всередині `ObjectEditor` на вертикальну навігацію (sidebar зліва від контенту)
- [Х] Вертикальні вкладки мають підтримувати довільну кількість секцій (буде рости з Phase 2+)
- [Х] Активна вкладка — візуально виділена (фон, border-left accent), всі інші — subtle
- [Х] Перелік секцій залежить від `kind` об'єкта. Мінімальний набір:
...
- [Х] Секція "Основні" — об'єднує displayName, ключові налаштування типу (те, що зараз у ObjectProperties TypeSettings)
- [Х] Секція "Дані" — вміщує дерево/список реквізитів, табличних частин, вимірів, ресурсів (структурний вигляд)
- [Х] Інші секції — наповнюватимуться у Phase 2+ (Форми, Макети, Права тощо)
- [Х] `activeSection` зберігається per-tab (Модуль B)
- [Х] Вертикальні вкладки мають ScrollArea для випадку великої кількості

### Clarify (питання перед імплементацією)
- [Х] Який мінімальний ширині sidebar-а вертикальних вкладок? 160px? 200px?
  ...
  - **Рішення: (A) фіксована ширина 160px (`w-40`).** Достатньо для label + icon, не забирає забагато простору.
...
### Definition of Done
- [Х] Вертикальна навігація відображається зліва від контенту в картці об'єкта
- [Х] Секції відповідають типу об'єкта
- [Х] Keyboard navigation працює (Arrow Up/Down, Enter)
- [Х] Scroll при великій кількості секцій
- [Х] Стилізація active/inactive секцій відповідає 1С-подібному UX
...
### Вимоги
- [Х] Розширити `buildTreeData` для побудови глибокого дерева:
  ```
  📁 Довідники (badge: 3)
    📋 Номенклатура
      📂 Реквізити
  ...
  📁 Регістри накопичення
    📋 ЗалишкиТоварів
      ...
      📂 Реквізити
  ```
- [Х] Структурні вузли (📂 Реквізити, 📂 Табличні частини, 📂 Виміри, 📂 Ресурси) — не editable, дії через контекстне меню (Додати)
- [Х] Вузли реквізитів (📄) — кліком виділяються, відкривають FieldProperties в правій панелі
- [Х] При додаванні нового реквізиту/табличної частини — автоматично:
  - Розгорнути батьківський вузол
  - Виділити новий елемент
  - Фокус на права панель для редагування
- [Х] Контекстне меню на реквізиті в дереві: Видалити, Вгору, Вниз
- [Х] Іконки для різних типів вузлів (hugeicons):
...
- [Х] Пошук (Ctrl+F) — фільтрує по всіх рівнях, включаючи імена реквізитів
- [Х] `expandedTreeNodes` — зберігати в ui-store, але використовувати react-arborist `ref` API для програмного розгортання при додаванні елементів
...
### Definition of Done
- [Х] Дерево відображає 4+ рівні: kind → object → structural group → field/section
- [Х] Клік на реквізит → FieldProperties в правій панелі з правильним контекстом (включаючи tabularSectionName)
- [Х] Додавання реквізиту → батько розгортається, новий елемент виділяється
- [Х] Пошук фільтрує по всіх рівнях
- [Х] Контекстне меню на реквізиті та структурній групі
- [Х] Іконки відповідають типам вузлів
...
### Вимоги
- [Х] Видалити стандартні реквізити з таблиці реквізитів у центральній зоні. Після створення об'єкта список custom реквізитів має бути **пустим**
- [Х] Додати в ObjectProperties (права панель) кнопку-посилання **"Стандартні реквізити"** (видимо тільки для types що мають стандартні реквізити — Catalog, Document, InformationRegister, AccumulationRegister, CustomTable; для Enumeration та Constant — не показувати)
...
- [ ] Стандартні реквізити табличних частин (id, line_number) — показувати в діалозі при виборі табличної частини в дереві (компонент готовий, потрібна інтеграція з деревом)
...
### Definition of Done
- [Х] Таблиця реквізитів у центральній зоні не містить стандартних реквізитів
- [Х] Кнопка “Стандартні реквізити” є в ObjectProperties
- [Х] Діалог показує актуальний список стандартних реквізитів для поточних settings
- [Х] Зміна hierarchyType → діалог одразу показує/ховає parent_id та is_folder
