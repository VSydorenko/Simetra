...
export class PostgresGenerator implements MetadataGenerator {
  ...
  generate(project: ProjectModel, options?: GeneratorOptions): GeneratorOutput {
    ...
    const opts: Required<GeneratorOptions> = {
      ...
      enumStrategy:
        options?.enumStrategy ??
        projectGen?.enumStrategy ??
        DEFAULT_OPTIONS.enumStrategy,
      constantsStrategy:
        options?.constantsStrategy ??
        projectGen?.constantsStrategy ??
        DEFAULT_OPTIONS.constantsStrategy,
