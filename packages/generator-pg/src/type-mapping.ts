...
export function mapFieldType(
  attr: { type: string; length?: number; precision?: number; scale?: number },
): string {
  switch (attr.type) {
    case 'UUID': return 'uuid'
    case 'String': return `varchar(${attr.length || 255})`
...
export function standardAttrToColumn(
  ...
  if (attr.type.startsWith('Enum(')) {
    ...
    return {
      ...
      constraints: [
        'NOT NULL',
        `CHECK (${attr.name} IN (${check}))`,
      ],
    }
  }

  if (attr.type === 'Ref') {
    return refToColumn(attr.ref, attr.allowedTypes, resolveRefTable, resolveEnumType)
  }
