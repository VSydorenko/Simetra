...
describe('focusWindow', () => {
  it('піднімає z-index вікна', () => {
    ...
    const zBefore = useUiStore.getState().floatingWindows.find(
      (w) => w.id === 'window-Catalog/A',
    )!.zIndex

    useUiStore.getState().focusWindow('window-Catalog/A')

    const zAfter = useUiStore.getState().floatingWindows.find(
      (w) => w.id === 'window-Catalog/A',
    )!.zIndex
...
describe('moveWindow / resizeWindow', () => {
  ...
  it('переміщує вікно', () => {
    useUiStore.getState().moveWindow('window-Catalog/Products', { x: 100, y: 200 })
  ...
  it('змінює розмір вікна', () => {
    useUiStore.getState().resizeWindow('window-Catalog/Products', { width: 800, height: 600 })
