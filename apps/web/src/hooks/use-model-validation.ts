...
import type { MetadataKind, MetadataObject, Attribute, ProjectModel } from '@simetra/core'
...
function refExists(model: ProjectModel, kind: MetadataKind, name: string): boolean {
...
function collectAllAttributes(obj: MetadataObject): { path: string; attr: Attribute }[] {
  ...
  if ('tabularSections' in obj && Array.isArray(obj.tabularSections)) {
    for (const ts of obj.tabularSections as { name: string; attributes: Attribute[] }[]) {
...
function validateSingleObject(model: ProjectModel, _kind: MetadataKind, obj: MetadataObject): ValidationError[] {
  ...
  if (!zodResult.success) {
    for (const issue of zodResult.error.issues) {
      errors.push({ path: resolveZodPath(obj, issue.path as (string | number)[]), message: issue.message })
    }
  }
  ...
  if ('recorderTypes' in obj && Array.isArray(obj.recorderTypes)) {
    for (const ref of obj.recorderTypes as { kind: MetadataKind; name: string }[]) {
      if (!refExists(model, ref.kind, ref.name)) {
        errors.push({ path: 'recorderTypes', message: `ref:${ref.kind}/${ref.name}` })
      }
    }
  }
  if ('registerMovements' in obj && Array.isArray(obj.registerMovements)) {
    for (const ref of obj.registerMovements as { kind: MetadataKind; name: string }[]) {
      if (!refExists(model, ref.kind, ref.name)) {
        errors.push({ path: 'registerMovements', message: `ref:${ref.kind}/${ref.name}` })
      }
    }
  }
      ...
      for (const kind of ALL_KINDS) {
        ...
        for (const [name, count] of nameCounts) {
          if (count > 1) {
            ...
            existing.push({ path: 'name', message: `Duplicate name "${name}" in ${kind}` })
